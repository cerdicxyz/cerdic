...
\title{
  \vspace{-1.5cm}
  \textbf{Synchra: Private, Leveraged FX on Arc}
  \vspace{-0.3cm}
}
...
\begin{abstract}
  \noindent Circle's Arc blockchain settles stablecoin value with sub-second finality and USDC-native gas, and Circle's own StableFX engine now settles \emph{spot} FX conversion between stablecoins on Arc~\cite{circlearc, circle_stablefx}. Neither provides \emph{leveraged} FX exposure, and neither hides who is trading what. Synchra is the layer above that: a clearing kernel that lets a trader post collateral once and open margined FX, crypto, and RWA positions against it, matched privately through a trusted execution environment (TEE) rather than a public order book. This paper describes the kernel, the portfolio margin model that makes leverage capital-efficient, the TEE matching design that makes it private, and where Synchra sits relative to existing onchain perpetuals (Ostium) and Arc's own settlement infrastructure (StableFX).
\end{abstract}
...
\textbf{Synchra} is a clearing kernel and margin engine that sits above Arc's settlement layer. It does three things:
...
FX is the first market because it is the one Arc's own roadmap is explicit about, and because Ostium already shows there is real demand for onchain leveraged FX/RWA exposure~\cite{ostium} -- just not privately, and not with portfolio margin. Section~\ref{sec:related} compares Synchra to both StableFX and Ostium directly.

\subsection{What this paper covers}

Section~\ref{sec:related} places Synchra against StableFX and Ostium concretely. Section~\ref{sec:model} defines actors and threat model. Section~\ref{sec:architecture} gives the layered design. Section~\ref{sec:clearing} is the clearing kernel; Section~\ref{sec:margin} is the portfolio margin formula and liquidation logic. Section~\ref{sec:markets} covers the market extension interface and the FX module specifically. Section~\ref{sec:privacy} is the TEE matching design. Section~\ref{sec:agents} and~\ref{sec:yield} cover agent accounts and yield-bearing collateral. Section~\ref{sec:economics} is the fee model. Section~\ref{sec:security} is a plain-language risk list. Section~\ref{sec:roadmap} is what ships first.
...
Rather than a generic DeFi literature survey, this section names the three systems Synchra sits between.

\begin{table}[h]
  \centering
  \begin{tabular}{lccc}
    \toprule
    & \textbf{StableFX} & \textbf{Ostium} & \textbf{Synchra} \\
    \midrule
    ...
    \bottomrule
  \end{tabular}
  \caption{Synchra relative to the two systems closest to it.}
  \label{tab:comparison}
\end{table}

\textbf{StableFX}~\cite{circle_stablefx} is Circle's own onchain FX engine: RFQ pricing aggregated across liquidity providers, payment-versus-payment settlement, live on Arc testnet since November 2025. It solves spot conversion. It has no leverage and no margin model, because it does not need one -- a spot conversion settles and is done. Synchra does not compete with StableFX; a Synchra FX position can settle its spot leg through it.
...
Synchra's position: take the demand Ostium has already validated, remove the transparency, and replace the pooled-LP counterparty with portfolio-margined, kernel-settled positions -- on the chain whose settlement layer (StableFX, USDC/EURC, CCTP) is purpose-built for exactly this instrument class.
...
\begin{figure}[h]
  ...
  \end{tikzpicture}
  \caption{Synchra layers. Only settlement is one-directional -- everything above it can query and be queried.}
...
EURC/USDC at launch, additional pairs after. Funding follows the interest-rate differential, $r_{\text{FX}} = r_{\text{quote}} - r_{\text{base}}$. The spot leg of a settled trade can route through StableFX's PvP settlement rather than requiring Synchra to bootstrap its own EURC/USDC liquidity. This is the module the rest of the paper's examples assume.
...
\begin{itemize}
  ...
  \item TEE-matched private RFQ against a real enclave (Intel TDX via Phala or Marlin), with on-chain attestation verification and public-settlement fallback.
...
Arc settles stablecoin value. StableFX settles spot FX conversion on top of it. Neither gives a trader leverage, and neither hides an order before it executes. Synchra is the layer that does both: post collateral once, get portfolio-margined leverage across correlated positions, match privately through a TEE, settle through the same rails Circle already built. Ostium already proved leveraged onchain FX/RWA has demand; Synchra's bet is that the next version of that market is private and portfolio-margined, on the chain built specifically for stablecoin-native FX.
