...
contract MockCollateralEngine is ICollateralEngine {
    ...
    function haircutBpsOf(address) external pure returns (uint16) {
        return 0;
    }

    function oraclePriceOf(address) external pure returns (uint256) {
        return 1e18;
    }

    function assetValueUsd(address, uint256) external pure returns (uint256) {
        return 0;
    }
...
contract OrderBookTest is Test {
    ...
    function test_PlaceOrderOutOfMarginReverts() public {
        ...
        vm.expectRevert(abi.encodeWithSelector(OrderBook.InsufficientMarginForOrder.selector, 10_000e18));
    ...
    function test_InvalidSignatureReverts() public {
        ...
        vm.expectRevert(abi.encodeWithSelector(OrderBook.InvalidSignature.selector, wrongAddr, trader));
        orderBook.placeOrder{value: RECYCLE_FEE}(MARKET_ID, SIDE, PRICE, SIZE, EXPIRY, sig);
    }
    ...
    function test_InsufficientBondReverts() public {
        ...
        vm.expectRevert(abi.encodeWithSelector(OrderBook.InsufficientBond.selector, 0.0005 ether, 0.001 ether));
    ...
    function test_ModifyOrderNotOwnerReverts() public {
        ...
        vm.prank(stranger);
        vm.expectRevert(abi.encodeWithSelector(OrderBook.NotOwner.selector, orderId, stranger));
    ...
    function test_ClaimRecycleFeeUnexpiredReverts() public {
        ...
        vm.expectRevert(abi.encodeWithSelector(OrderBook.OrderNotExpired.selector, orderId, EXPIRY, block.number));
        orderBook.claimRecycleFee(orderId);
    }
    ...
    function test_ModifyExpiredOrderReverts() public {
        ...
        vm.prank(trader);
        vm.expectRevert(abi.encodeWithSelector(OrderBook.OrderExpired.selector, orderId, expiry, block.number));
        orderBook.modifyOrder(orderId, 110_000e18, 2e18);
    }
}
