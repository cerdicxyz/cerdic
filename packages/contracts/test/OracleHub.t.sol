...
contract OracleHubTest is Test {
    ...
    function test_PythRevertsOnStalePrice() public {
        ...
        vm.expectRevert(abi.encodeWithSelector(PythConsumer.StalePrice.selector, BTC_USD_FEED, publishTime, block.timestamp));
    ...
    function test_PythRevertsOnFutureDatedPrice() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(PythConsumer.StalePrice.selector, BTC_USD_FEED, block.timestamp + 1_000, block.timestamp)
        );
        pythConsumer.fetchPrice(BTC_USD_FEED);
    }
    ...
    function test_ChainlinkRevertsOnStalePrice() public {
        ...
        vm.warp(block.timestamp + 61);
        vm.expectRevert(
            abi.encodeWithSelector(ChainlinkConsumer.StalePrice.selector, address(aggregator), updatedAt, block.timestamp)
        );
    ...
    function test_ChainlinkRevertsOnNonPositiveAnswer() public {
        ...
        vm.expectRevert(abi.encodeWithSelector(ChainlinkConsumer.InvalidPrice.selector, address(aggregator), int256(-1)));
        chainlinkConsumer.fetchPrice(address(aggregator));
    }
