...
contract ProtocolConstantsTest is Test {
    ...
    function test_mmrIs60PercentOfImr() public view {
        ...
        assertEq(
            constants.mmrBps() * 100,
            constants.imrBps() * 60,
            "MMR must equal 60% of IMR"
        );
    }

    function test_liquidationGammaPercent() public view {
        assertEq(
            constants.liquidationGammaPercent(),
            85,
            "LIQUIDATION_GAMMA_PERCENT must be 85 (gamma=0.85)"
        );
    }

    function test_maxLeverageBps() public view {
        assertEq(
            constants.maxLeverageBps(),
            2000,
            "MAX_LEVERAGE_BPS must be 20x (2000 bps)"
        );
    }
    ...
    function test_fundingMaxRateBpsPerSec() public view {
        assertEq(
            constants.fundingMaxRateBpsPerSec(),
            30,
            "FUNDING_MAX_RATE_BPS_PER_SEC must be 30 (MVP conservative pick)"
        );
    }

    function test_t1HaircutBps() public view {
        assertEq(
            constants.t1HaircutBps(), 0, "T1_HAIRCUT_BPS must be 0 bps"
        );
    }

    function test_t2HaircutBpsMin() public view {
        assertEq(
            constants.t2HaircutBpsMin(),
            200,
            "T2_HAIRCUT_BPS_MIN must be 2% (200 bps)"
        );
    }

    function test_t2HaircutBpsMax() public view {
        assertEq(
            constants.t2HaircutBpsMax(),
            500,
            "T2_HAIRCUT_BPS_MAX must be 5% (500 bps)"
        );
    }

    function test_t3HaircutBpsMin() public view {
        assertEq(
            constants.t3HaircutBpsMin(),
            1000,
            "T3_HAIRCUT_BPS_MIN must be 10% (1000 bps)"
        );
    }

    function test_t3HaircutBpsMax() public view {
        assertEq(
            constants.t3HaircutBpsMax(),
            2000,
            "T3_HAIRCUT_BPS_MAX must be 20% (2000 bps)"
        );
    }

    function test_t4HaircutBpsMin() public view {
        assertEq(
            constants.t4HaircutBpsMin(),
            1500,
            "T4_HAIRCUT_BPS_MIN must be 15% (1500 bps)"
        );
    }

    function test_t4HaircutBpsMax() public view {
        assertEq(
            constants.t4HaircutBpsMax(),
            3500,
            "T4_HAIRCUT_BPS_MAX must be 35% (3500 bps)"
        );
    }
    ...
    function test_tierMonotonicity_min() public view {
        ...
        assertLt(
            constants.t2HaircutBpsMin(),
            constants.t3HaircutBpsMin(),
            "T2 min must be < T3 min"
        );
        assertLt(
            constants.t3HaircutBpsMin(),
            constants.t4HaircutBpsMin(),
            "T3 min must be < T4 min"
        );
    }
    ...
    function test_tierMonotonicity_max() public view {
        ...
        assertLt(
            constants.t2HaircutBpsMax(),
            constants.t3HaircutBpsMax(),
            "T2 max must be < T3 max"
        );
        assertLt(
            constants.t3HaircutBpsMax(),
            constants.t4HaircutBpsMax(),
            "T3 max must be < T4 max"
        );
    }
    ...
    function test_tierRangesAreOrdered() public view {
        ...
        assertLt(
            constants.t2HaircutBpsMin(),
            constants.t2HaircutBpsMax(),
            "T2 min must be < T2 max"
        );
        assertLt(
            constants.t3HaircutBpsMin(),
            constants.t3HaircutBpsMax(),
            "T3 min must be < T3 max"
        );
        assertLt(
            constants.t4HaircutBpsMin(),
            constants.t4HaircutBpsMax(),
            "T4 min must be < T4 max"
        );
    }
}
