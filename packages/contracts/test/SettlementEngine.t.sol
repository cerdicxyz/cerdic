...
contract SettlementEngineTest is Test {
    ...
    function test_ValidateOpenFalseShortIsAtomic() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(SettlementEngine.InsufficientMargin.selector, shortTrader, -SIZE, MARGIN)
        );
        vm.prank(admin);
        engine.settleTrade(MARKET_ID, longTrader, shortTrader, SIZE, PRICE, 0);
    ...
    function test_PremiumTransferFailureIsAtomic() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(SettlementEngine.PremiumTransferFailed.selector, rejectingSeller, premium)
        );
        vm.prank(admin);
