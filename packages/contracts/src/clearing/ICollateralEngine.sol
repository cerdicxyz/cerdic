...
interface ICollateralBalanceSource {
    ...
    function collateralBalanceOf(address trader, address asset)
        external
        view
        returns (uint256);
}
...
interface ICollateralEngine {
    ...
    function assetValueUsd(address asset, uint256 amount)
        external
        view
        returns (uint256);
    ...
    function effectiveCollateral(address trader)
        external
        view
        returns (uint256);
    ...
    function registerAsset(address asset, uint8 tier, uint16 haircutBps)
        external;
}
