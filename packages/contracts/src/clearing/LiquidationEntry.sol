...
contract LiquidationEntry {
    ...
    function executeStandardLiquidation(address trader, bytes32 marketId, uint256 maxNotional) external {
        ...
        emit StandardLiquidationExecuted(trader, marketId, msg.sender, closeSizeAbs, closedNotional, penaltyUsd - unpaid);
    }
    ...
    function _settleTakeover(address trader, bytes32 marketId, int256 signedSize, uint256 closeSizeAbs, uint256 markPrice)
        internal
    {
    ...
    ///      actually seized.
    function _seizeAssetChunk(address trader, address liquidator, address insuranceRecipient, address asset, uint256 remainingUsd)
        internal
        returns (uint256 seizedUsd)
    {
