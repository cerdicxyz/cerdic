...
contract OrderBook {
    ...
    bytes32 internal constant SIDE_TYPEHASH =
        keccak256("Side(uint8 id)");
    ...
    function placeOrder(
        ...
        bytes32 digest;
        {
            ...
            bytes32 structHash = keccak256(
                abi.encode(ORDER_TYPEHASH, marketId, side, price, size, expiryBlock, nonce)
            );
    ...
    function _resolveMarket(bytes32 marketId) internal view returns (address) {
        (bool ok, bytes memory data) = positionEngine.staticcall(
            abi.encodeWithSignature("positionDecoders(bytes32)", marketId)
        );
    ...
    function _effectiveCollateral(address trader) internal view returns (uint256) {
        (bool ok, bytes memory data) = collateralEngine.staticcall(
            abi.encodeWithSignature("effectiveCollateral(address)", trader)
        );
